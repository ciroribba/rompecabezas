...
var instrucciones = ['Utilizar botones "Flechas" del teclado para jugar',
'Las piezas se mueven en sentido hacia el espacio vacio',
'Por ejemplo Flecha "→" mueve pieza hacia derecha si el espacio esta vacio'];
...
/*creé esta variable para comparar, ya que la primera se sobreescribe*/
var grillaFinal = [
  [1, 2, 3],
  [4, 5, 6],
  [7, 8, 9]
...
/*Codigo de funcionalidades extras creadas*/

/*Codigo de acciones una vez que se compruebe que se ha ganado*/
function hasGanado()
{
    document.getElementById('section-uno').style.display='none';
    document.getElementById('section-dos').style.display='block';
    var body = document.getElementsByTagName('body')[0]; 
    body.style.backgroundImage = 'url(images/blanco.png)'; 
};

/*Funcion que lanza el juego una vez que el usuario selecciono un puzzle*/ 
function jugar()
{
	document.getElementById('intro').style.display='none';
    document.getElementById('section-uno').style.display='block';
    document.getElementById('section-dos').style.display='none';        
};
/*Muestro contenido principal segun eleccion de puzzle del usuario
 las opciones de rompecabezas.*/
var ids=['maggie','bart','lisa'];
...
/*creo las rutas de las imagenes segun la eleccion del puzzle del usuario.*/
    var maggie = 1;
    var bart = 10;
    var lisa = 100;

function crearImgCortada(personaje){
	for(var i = 1; i<9; i++){
		document.getElementById("img"+i).src="images/"+(i*personaje)+".jpg";
	}
};
  

/*Reuno en una funcion principal el resto de las funciones previas, mas la de inciar
en orden de acuerdo a la logica del programa para que funcione correctamente*/
function lanzarJuego(variablePersonaje,stringPersonaje){
document.getElementById('demo').play();    
jugar();
crearImgCortada(variablePersonaje);
mostrar(stringPersonaje);
iniciar();
};
/*
 fin codigo mio extra
 */
...
function posicionValida(fila, columna) {
    //COMPLETADO!!!
    if(fila<0 || fila>(grilla.length-1) || columna<0 || columna>(grilla.length-1)){
      return false;
    }
    return true;
}
...
function moverEnDireccion(direccion) {
  ...
  //COMPLETAR: Agregar la dirección del movimiento al arreglo de movimientos
        direction(direccion);//COMPLETADO!!!
    }
}
